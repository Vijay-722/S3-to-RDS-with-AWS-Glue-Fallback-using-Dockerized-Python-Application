...
This project is a **Dockerized Python application** that automates data ingestion from **Amazon S3 to RDS (MySQL)** with a built-in **fallback to AWS Glue** when RDS is unavaiable. It ensures **reliable, fault-tolerant data pipelines** using core AWS services--**S3, RDS, Glue, and IAM**.

The app reads CSV files from S3, uploads them to RDS, and if the process fails, it registers the dataset in the **Glue Data Catalog**, maintaining continuous data availability for analytics.

- 🚀 Automated data ingestion pipeline
- ☁️ Seamless AWS service integration (S3, RDS, Glue)
- 🐳 Dockerized for consistent deployment
- 🔁 Automatic Glue fallback on RDS failure
- 📊 Schema inference and Glue Data Catalog registration
- 🧩 Built using Python (pandas, boto3, SQLAlchemy, PyMySQL)
...
1. Create a bucket (e.g., `s3-rds-bucket-18`)
2. Upload `input.csv`:
...
GLUE_S3_LOCATION=s3://s3-rds-bucket-18/
```
